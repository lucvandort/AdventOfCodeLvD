...
checksum = len(identifiers_with_double_letters) * len(identifiers_with_triple_letters)
...
identifier_comparison.loc[:,['id1', 'id2']] = list(combinations(identifier_input.loc[:,0], r))
...
best_match = identifier_comparison[identifier_comparison.loc[:,'num_changes'] == 1]
match_ndiff = best_match.loc[:,'ndiff'].values[0]
final_id = ''.join([c for c in match_ndiff if '+' not in c and '-' not in c]).replace(' ', '')
